...
<?php 
	...
	include_once ($filepath.'/../classes/history.php');
 ...
 <?php
    ...
    if(isset($_GET['shiftid'])){
		...
		$insert_history = $ht->insert_history($cusid,$proid,$qty);
    	// if($get_email){
                 ...
                 ?> 
					...
					<tbody>
						...
						<tr class="odd gradeX">
							...
							<td><?php   $cus = new customer();
										$get_customer_name = $cus -> show_customers($result['customer_id']);
										$result2 = $get_customer_name->fetch_assoc();
										?>
										
										<a href="customer.php?id=<?php echo $result['customer_id'] ?>"><?php echo $result2['name'] ?></a>
							</td>
							...
							<td>
								...
								if($result['status']==0){
								 ?>
								<a href="?shiftid=<?php echo $result['orderId']; ?>&qty=<?php echo $result['quantity']; ?>&proid=<?php echo $result['productId']; ?>&price=<?php echo $result['price']; ?>&time=<?php echo $result['date_order']; ?>&cusid=<?php echo $result['customer_id']; ?>">Đang chờ xử lý</a>
								...
								<?php 
								 ...
								 ?>
								<a href="?confirmid=<?php echo $result['orderId'] ?>&price=<?php echo $result['price']; ?>&time=<?php echo $result['date_order'] ?>">Đã gửi hàng</a>
								 <?php 
